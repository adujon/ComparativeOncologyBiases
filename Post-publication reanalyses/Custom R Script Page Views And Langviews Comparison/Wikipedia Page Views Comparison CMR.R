...
png("Custom VS Langviews Wikipedia correlation CMR.png", width = 20, height = 20, unit = "cm",res = 300)
p_comp
dev.off()
...
fit_logNViews_mortality_langviews  <- brm(NMortality | trials(NNecropsies)  ~ logNViews_Langviews + (1|gr(Species, cov = vcv_brms_OU)),
                               family =binomial(),
                               data = data_mortality,
                               data2 = list(vcv_brms_OU = vcv_brms_OU_mortality), 
                               chains = 4,
...
fit_logNViews_mortality_langviews 

#calculate odd ratios for mortality
rsq <- rstantools::bayes_R2(fit_logNViews_mortality_langviews , re.form = NA)
fe <- format(round(exp(fixed.effects(fit_logNViews_mortality_langviews)), digits = 1), nsmall = 1)
OR <- paste("OR = ", fe[2,1], " (95% CI:", fe[2,3], ", ", fe[2,4], ")", sep = "")
rsq_lbl <- as.expression(bquote(~.(OR)))
es_mortality<-  data.frame(OR  = fe[2,1], Q2.5 = fe[2,3], Q97.5 = fe[2,4], Variable = "log10 Number of views", Tumour = "Mortality")
effect_sizes <- rbind(effect_sizes, es_mortality)
...
#calculate marginal effect for mortality
cond_effects <- conditional_effects(fit_logNViews_mortality_langviews , effect = "logNViews_Langviews", resp = "NMortality")
cond_effects_plot <- data.frame(cond_effects[[1]])
cond_effects_plot$estimate__ <- cond_effects_plot$estimate__/cond_effects_plot$NNecropsies[1]
cond_effects_plot$lower__ <- cond_effects_plot$lower__/cond_effects_plot$NNecropsies[1]
cond_effects_plot$upper__ <- cond_effects_plot$upper__/cond_effects_plot$NNecropsies[1]
...
png("Custom VS Langviews Wikipedia views model comparison CMR.png", width = 40, height = 20, unit = "cm",res = 300)
pfigure
dev.off()